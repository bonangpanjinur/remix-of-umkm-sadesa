-- ============================================================
-- DESA WISATA & UMKM PLATFORM - MAIN MIGRATION (CONSOLIDATED)
-- Generated: 2026-03-11
-- Versi: 7.0 - Fully Idempotent
-- Deskripsi: Satu file SQL utama untuk setup/migrasi database.
--            Aman dijalankan pada database baru maupun yang sudah ada.
-- ============================================================
-- Strategi Idempoten:
...
-- FK constraint untuk merchants.current_subscription_id
DO $$ BEGIN
...
-- 3.23 WITHDRAWAL REQUESTS (Merchant)
...
-- 3.25 COURIER WITHDRAWAL REQUESTS
...
-- 3.55 RIDE REQUESTS (from later migration)
...
-- 3.56 MERCHANT GALLERY (from later migration)
...
-- 3.57 COURIER DEPOSITS (from later migration)
...
-- 3.58 COURIER BALANCE LOGS (from later migration)
...
-- 3.59 MERCHANT FAVORITES (from later migration)
...
-- 6.3 Chat functions
CREATE OR REPLACE FUNCTION public.is_chat_participant(_user_id uuid, _order_id uuid, _chat_type text DEFAULT 'buyer_merchant')
RETURNS boolean LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path TO 'public' AS $$
BEGIN
  IF _chat_type = 'buyer_merchant' THEN
    RETURN EXISTS (
      SELECT 1 FROM orders o LEFT JOIN merchants m ON o.merchant_id = m.id
      WHERE o.id = _order_id AND (o.buyer_id = _user_id OR m.user_id = _user_id)
    );
  ELSIF _chat_type = 'buyer_courier' THEN
    RETURN EXISTS (
      SELECT 1 FROM orders o LEFT JOIN couriers c ON o.courier_id = c.id
      WHERE o.id = _order_id AND (o.buyer_id = _user_id OR c.user_id = _user_id)
    );
  ELSIF _chat_type = 'merchant_courier' THEN
    RETURN EXISTS (
      SELECT 1 FROM orders o LEFT JOIN merchants m ON o.merchant_id = m.id LEFT JOIN couriers c ON o.courier_id = c.id
      WHERE o.id = _order_id AND (m.user_id = _user_id OR c.user_id = _user_id)
    );
  END IF;
  RETURN FALSE;
END;
...
BEGIN
  DELETE FROM public.chat_messages WHERE auto_delete_at IS NOT NULL AND auto_delete_at <= NOW();
  GET DIAGNOSTICS deleted_count = ROW_COUNT;
  RETURN deleted_count;
END;
$$;

-- 6.4 Business logic functions
...
BEGIN
  SELECT * INTO v_ride FROM ride_requests WHERE id = p_ride_id AND status = 'SEARCHING' FOR UPDATE;
  IF NOT FOUND THEN RETURN jsonb_build_object('success', false, 'message', 'Ride not found or already taken'); END IF;
  UPDATE ride_requests SET courier_id = p_courier_id, status = 'ACCEPTED', accepted_at = now(), updated_at = now() WHERE id = p_ride_id;
  RETURN jsonb_build_object('success', true, 'message', 'Ride accepted');
END;
$$;

-- 6.6 Admin RPCs for quota management
...
BEGIN
  ...
  IF v_sub.payment_status = 'PAID' AND v_sub.status = 'ACTIVE' THEN RETURN json_build_object('success', false, 'message', 'Subscription sudah aktif'); END IF;
...
RETURNS JSON LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  IF p_admin_notes IS NULL OR p_admin_notes = '' THEN RETURN json_build_object('success', false, 'message', 'Catatan admin wajib diisi saat menolak'); END IF;
...
BEGIN
  ...
  WHERE p_product_price >= min_price AND (max_price IS NULL OR p_product_price <= max_price) AND is_active = true
...
-- ============================================================
-- BAGIAN 7: ENABLE ROW LEVEL SECURITY (semua 59 tabel)
-- ============================================================
...
-- ============================================================
-- BAGIAN 8: RLS POLICIES (Drop semua → Recreate)
-- ============================================================

-- Drop semua policy yang ada di public schema
DO $$
DECLARE r RECORD;
BEGIN
...
CREATE POLICY "Admins can view all roles" ON public.user_roles FOR SELECT USING (is_admin());
CREATE POLICY "Admins can insert user roles" ON public.user_roles FOR INSERT WITH CHECK (is_admin());
CREATE POLICY "Admins can update user roles" ON public.user_roles FOR UPDATE USING (is_admin()) WITH CHECK (is_admin());
CREATE POLICY "Admins can delete user roles" ON public.user_roles FOR DELETE USING (is_admin());

-- === VILLAGES ===
CREATE POLICY "Anyone can view active villages" ON public.villages FOR SELECT USING (is_active = true);
CREATE POLICY "Village owners can view own village" ON public.villages FOR SELECT USING (user_id = auth.uid());
...
CREATE POLICY "Authenticated users view active merchants" ON public.merchants FOR SELECT USING ((user_id = auth.uid()) OR ((status = 'ACTIVE' AND registration_status = 'APPROVED' AND auth.uid() IS NOT NULL)) OR is_admin() OR is_verifikator());
...
CREATE POLICY "Merchants can manage own products" ON public.products FOR ALL USING (EXISTS (SELECT 1 FROM merchants WHERE merchants.id = products.merchant_id AND merchants.user_id = auth.uid())) WITH CHECK (EXISTS (SELECT 1 FROM merchants WHERE merchants.id = products.merchant_id AND merchants.user_id = auth.uid()));
...
CREATE POLICY "Admin desa can manage tourism" ON public.tourism FOR ALL USING (EXISTS (SELECT 1 FROM villages WHERE villages.id = tourism.village_id AND villages.registration_status = 'APPROVED') AND is_admin_desa()) WITH CHECK (EXISTS (SELECT 1 FROM villages WHERE villages.id = tourism.village_id AND villages.registration_status = 'APPROVED') AND is_admin_desa());
...
CREATE POLICY "Merchants can view order items" ON public.order_items FOR SELECT USING (EXISTS (SELECT 1 FROM orders o JOIN merchants m ON o.merchant_id = m.id WHERE o.id = order_items.order_id AND m.user_id = auth.uid()));
CREATE POLICY "Couriers can view order items" ON public.order_items FOR SELECT USING (EXISTS (SELECT 1 FROM orders o JOIN couriers c ON o.courier_id = c.id WHERE o.id = order_items.order_id AND c.user_id = auth.uid()));
...
CREATE POLICY "Merchants can view refunds for their orders" ON public.refund_requests FOR SELECT USING (EXISTS (SELECT 1 FROM orders o JOIN merchants m ON o.merchant_id = m.id WHERE o.id = refund_requests.order_id AND m.user_id = auth.uid()));
...
-- === COURIER WITHDRAWAL REQUESTS ===
CREATE POLICY "Admins full access courier withdrawals" ON public.courier_withdrawal_requests FOR ALL USING (is_admin());
CREATE POLICY "Couriers can view own withdrawals" ON public.courier_withdrawal_requests FOR SELECT USING (courier_id = get_user_courier_id());
CREATE POLICY "Couriers can create own withdrawals" ON public.courier_withdrawal_requests FOR INSERT WITH CHECK (courier_id = get_user_courier_id());
...
CREATE POLICY "Admins manage withdrawals" ON public.verifikator_withdrawals FOR ALL USING (is_admin());
...
CREATE POLICY "Verifikators view own withdrawals" ON public.verifikator_withdrawals FOR SELECT USING (verifikator_id = auth.uid() OR is_admin());

-- === GROUP ANNOUNCEMENTS ===
CREATE POLICY "Verifikator can manage own announcements" ON public.group_announcements FOR ALL USING (verifikator_id = auth.uid());
CREATE POLICY "Group members can read announcements" ON public.group_announcements FOR SELECT USING (EXISTS (SELECT 1 FROM group_members gm JOIN merchants m ON m.id = gm.merchant_id WHERE gm.group_id = group_announcements.group_id AND m.user_id = auth.uid()));
...
-- === CHAT MESSAGES ===
CREATE POLICY "Users can view own chat messages" ON public.chat_messages FOR SELECT USING (auth.uid() = sender_id OR auth.uid() = receiver_id OR is_chat_participant(auth.uid(), order_id, chat_type));
CREATE POLICY "Users can send chat messages" ON public.chat_messages FOR INSERT WITH CHECK (auth.uid() = sender_id AND is_chat_participant(auth.uid(), order_id, chat_type));
CREATE POLICY "Users can update own messages read status" ON public.chat_messages FOR UPDATE USING (auth.uid() = receiver_id);
CREATE POLICY "Admins can manage all chat messages" ON public.chat_messages FOR ALL USING (is_admin());
...
-- === RATE LIMITS ===
CREATE POLICY "Rate limits managed by functions" ON public.rate_limits FOR SELECT USING (identifier = auth.uid()::text OR is_admin());
...
CREATE POLICY "Merchants can view own page views" ON public.page_views FOR SELECT USING (EXISTS (SELECT 1 FROM merchants WHERE merchants.id = page_views.merchant_id AND merchants.user_id = auth.uid()) OR is_admin());

-- === HALAL REGULATIONS ===
CREATE POLICY "Anyone can read halal_regulations" ON public.halal_regulations FOR SELECT USING (true);
CREATE POLICY "Admin can manage halal_regulations" ON public.halal_regulations FOR ALL USING (is_admin());

-- === POS PACKAGES ===
CREATE POLICY "Anyone can view active POS packages" ON public.pos_packages FOR SELECT USING (is_active = true);
CREATE POLICY "Admins can manage POS packages" ON public.pos_packages FOR ALL USING (is_admin());

-- === POS SETTINGS ===
CREATE POLICY "Merchants can manage own POS settings" ON public.pos_settings FOR ALL USING (EXISTS (SELECT 1 FROM merchants WHERE merchants.id = pos_settings.merchant_id AND merchants.user_id = auth.uid()));
CREATE POLICY "Admins can manage all POS settings" ON public.pos_settings FOR ALL USING (is_admin());

-- === POS SUBSCRIPTIONS ===
CREATE POLICY "Merchants can view own POS subscriptions" ON public.pos_subscriptions FOR SELECT USING (EXISTS (SELECT 1 FROM merchants WHERE merchants.id = pos_subscriptions.merchant_id AND merchants.user_id = auth.uid()));
CREATE POLICY "Merchants can create POS subscriptions" ON public.pos_subscriptions FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM merchants WHERE merchants.id = pos_subscriptions.merchant_id AND merchants.user_id = auth.uid()));
CREATE POLICY "Merchants can update own POS subscriptions" ON public.pos_subscriptions FOR UPDATE USING (EXISTS (SELECT 1 FROM merchants WHERE merchants.id = pos_subscriptions.merchant_id AND merchants.user_id = auth.uid()));
CREATE POLICY "Admins can manage all POS subscriptions" ON public.pos_subscriptions FOR ALL USING (is_admin());

-- === POS TRANSACTIONS ===
CREATE POLICY "Merchants can manage own POS transactions" ON public.pos_transactions FOR ALL USING (EXISTS (SELECT 1 FROM merchants WHERE merchants.id = pos_transactions.merchant_id AND merchants.user_id = auth.uid()));
CREATE POLICY "Admins can view all POS transactions" ON public.pos_transactions FOR SELECT USING (is_admin());

-- === RIDE REQUESTS ===
CREATE POLICY "Passengers can create ride requests" ON public.ride_requests FOR INSERT TO authenticated WITH CHECK (passenger_id = auth.uid());
CREATE POLICY "Passengers can view own rides" ON public.ride_requests FOR SELECT TO authenticated USING (passenger_id = auth.uid());
CREATE POLICY "Couriers can view searching rides" ON public.ride_requests FOR SELECT TO authenticated USING (status = 'SEARCHING' AND EXISTS (SELECT 1 FROM couriers WHERE user_id = auth.uid() AND status = 'ACTIVE' AND is_available = true));
CREATE POLICY "Couriers can view assigned rides" ON public.ride_requests FOR SELECT TO authenticated USING (courier_id IN (SELECT id FROM couriers WHERE user_id = auth.uid()));
CREATE POLICY "Couriers can update assigned rides" ON public.ride_requests FOR UPDATE TO authenticated USING (courier_id IN (SELECT id FROM couriers WHERE user_id = auth.uid()));
CREATE POLICY "Admins can manage all rides" ON public.ride_requests FOR ALL TO authenticated USING (has_role(auth.uid(), 'admin'));
CREATE POLICY "Passengers can update own rides" ON public.ride_requests FOR UPDATE TO authenticated USING (passenger_id = auth.uid() AND status IN ('SEARCHING', 'ACCEPTED'));

-- === MERCHANT GALLERY ===
CREATE POLICY "Public can view merchant gallery" ON public.merchant_gallery FOR SELECT USING (EXISTS (SELECT 1 FROM merchants WHERE merchants.id = merchant_gallery.merchant_id AND merchants.status = 'ACTIVE' AND merchants.registration_status = 'APPROVED'));
CREATE POLICY "Merchants can manage own gallery" ON public.merchant_gallery FOR ALL USING (EXISTS (SELECT 1 FROM merchants WHERE merchants.id = merchant_gallery.merchant_id AND merchants.user_id = auth.uid()));
CREATE POLICY "Admins can manage all galleries" ON public.merchant_gallery FOR ALL USING (is_admin());

-- === COURIER DEPOSITS ===
CREATE POLICY "Couriers can view own deposits" ON public.courier_deposits FOR SELECT USING (courier_id = get_user_courier_id());
CREATE POLICY "Couriers can create own deposits" ON public.courier_deposits FOR INSERT WITH CHECK (courier_id = get_user_courier_id());
CREATE POLICY "Admins full access courier deposits" ON public.courier_deposits FOR ALL USING (is_admin());

-- === COURIER BALANCE LOGS ===
CREATE POLICY "Couriers can view own balance logs" ON public.courier_balance_logs FOR SELECT USING (courier_id = get_user_courier_id());
CREATE POLICY "Admins full access balance logs" ON public.courier_balance_logs FOR ALL USING (is_admin());
...
CREATE TRIGGER update_ride_requests_updated_at BEFORE UPDATE ON public.ride_requests FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
DROP TRIGGER IF EXISTS trigger_set_chat_auto_delete ON public.chat_messages;
CREATE TRIGGER trigger_set_chat_auto_delete BEFORE INSERT ON public.chat_messages FOR EACH ROW EXECUTE FUNCTION set_chat_auto_delete();
...
INSERT INTO storage.buckets (id, name, public) VALUES ('merchants', 'merchants', true) ON CONFLICT (id) DO NOTHING;
...
DO $$ BEGIN ALTER PUBLICATION supabase_realtime ADD TABLE public.courier_deposits; EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN ALTER PUBLICATION supabase_realtime ADD TABLE public.courier_balance_logs; EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ============================================================
-- BAGIAN 12: DEFAULT DATA (Seed)
-- ============================================================
...
INSERT INTO public.app_settings (key, value, category, description) VALUES
  ...
  ('ride_fare_settings', '{"base_fare": 5000, "per_km_fare": 3000, "minimum_fare": 8000}', 'ride', 'Pengaturan tarif ojek'),
  ('courier_minimum_balance', '{"minimum_balance": 50000}', 'courier', 'Saldo minimum kurir untuk terima COD')
